...
class Spider(Spider):
    ...
    def _request_with_failover(self, path, timeout=(3, 5)):
        ...
        for domain in urls_to_try:
            ...
            full_url = f'{clean_domain}{path}' if path.startswith('/') else f'{clean_domain}/{path}'
            headers = {
                **self.headers,
                'Origin': clean_domain,
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log(f'[HOST] 域名 {clean_domain} 请求失败，状态码: {response.status_code}，尝试下一个...')
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        try:
            ...
            if id.startswith('lemon'):
                ...
                rsp = self.session_get(f'https://edge-hls.sacfedge.com/hls/{sid}/master/{sid}_auto.m3u8?playlistType=lowLatency').text
                lines = rsp.strip().split('\n')
                    ...
                    if '#EXT-X-STREAM-INF' in line:
                        ...
                        if bandwidth_match:
                            ...
                            if bw > max_bandwidth:
                                ...
                                best_url = lines[i + 1]
                if best_url:
                    full_url = f'{best_url}&psch={psch}&pkey={pkey}&preferredVideoCodec={self.stripchat_preferredVideoCodec}'
                    urls = ["最高画质", f'{self.getProxyUrl()}&url={quote(full_url)}']
            else:
    ...
    def localProxy(self, param):
        ...
        if rsp.status_code == 403:
            rsp = self.session_get(re.sub(r'(_\d+p\d*)?\.m3u8', '_160p_blurred.m3u8', url))
    ...
    def replace_emoji(self, text: str) -> str:
        emoji_map = {
            ':heart:': '❤️', ':dancing:': '💃', ':thumbsup:': '👍', ':flower:': '🌹', ':lol:': '😄',
    ...
    def send_live_danmaku(self, item):
        try:
            ...
            self.call_local_action(f'do=danmaku&text={quote(show)}', f'实时弹幕发送: {show}')
        except Exception as e:
